...
9. **Comment cap**: the pipeline fetches at most a couple hundred comments per YouTube video (`COMMENTS_MAX_COMMENTS`, default 200 in code) and the AI sees at most 30 (`AI_MAX_COMMENTS`). Neither number appears in the public docs; developers will assume "comments" means the thread. The skills say "capped to the top couple hundred".
...
11. **Description artifact naming** differs between the docs (`description.txt`) and the pipeline output seen in the mock (`video.description`, and `thumbnail.description` in `argBuilder.ts`). The skills point at `summary.descriptionFile` instead of a hard-coded name.
