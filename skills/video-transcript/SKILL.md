...
| `HTTP Error 429` while fetching subtitles | Caption endpoint rate limit. Almost always triggered by requesting **translated** tracks (`en-orig` → `fr`) or too many tracks | Request only native tracks (`--list-subs` shows which are native); add `--sleep-subtitles 60` (the maintainers' tested workaround; a PO token does not help here); translate the text yourself |
| One 429'd language fails the whole run | yt-dlp aborts the invocation if any requested sub-lang fails | Ask for exactly one language per invocation |
